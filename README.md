...
- **輸注速率警示（v0.4.3）**：自訂試算的輸注時長低於 `max(60 min, 劑量÷10 mg/min)` 時警示（1g→1.7h、2g→3.3h；仿單速率上限與「最短 60 分鐘」兩條規則取長）。**僅屬給藥安全**——AUC=每日總量/CL 不受 tInf 影響，故不降信心、不擋劑量建議。目前僅涵蓋 Mode 3 自訂試算，Mode 1/2 的輸注時長輸入尚未檢查
...
node js/safety.test.js       # 安全層行為（BLOCK/WARNING 觸發正確性、輸注速率）47/47
